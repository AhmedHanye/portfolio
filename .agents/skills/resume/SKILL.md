...
```bash
cd resume && uvx --from "rendercv[full]" rendercv render resume.yaml -o dist && cp dist/Ahmed_Hanye_CV.pdf ../public/resume.pdf
```
...
```bash
pdftotext -f 1 -l 2 resume/dist/Ahmed_Hanye_CV.pdf - | head -n 40
```
